...
func (s *Server) DoUnaryUnary(ctx context.Context, msg *Message) (*Message, error) {
	defer oldfritter.StartSegment(oldfritter.FromContext(ctx), "DoUnaryUnary").End()
...
func (s *Server) DoUnaryStream(msg *Message, stream TestApplication_DoUnaryStreamServer) error {
	defer oldfritter.StartSegment(oldfritter.FromContext(stream.Context()), "DoUnaryStream").End()
...
func (s *Server) DoStreamUnary(stream TestApplication_DoStreamUnaryServer) error {
	defer oldfritter.StartSegment(oldfritter.FromContext(stream.Context()), "DoStreamUnary").End()
	md, _ := metadata.FromIncomingContext(stream.Context())
	js, _ := json.Marshal(md)
	for {
		_, err := stream.Recv()
...
func (s *Server) DoStreamStream(stream TestApplication_DoStreamStreamServer) error {
	defer oldfritter.StartSegment(oldfritter.FromContext(stream.Context()), "DoStreamStream").End()
	md, _ := metadata.FromIncomingContext(stream.Context())
	js, _ := json.Marshal(md)
	for {
		_, err := stream.Recv()
